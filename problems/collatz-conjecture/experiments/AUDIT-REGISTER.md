...
3. **Data memo filed (2026-07-02): per-event regeneration statistics at 10⁷-start scale.** [`REGENERATION-EVENTS.md`](REGENERATION-EVENTS.md) (instrument `regeneration_events_scan.py`, 1.12×10⁹ orbit states). Two independent referees (overclaim; data/code — the latter re-implemented the scan on a disjoint range and recomputed both max-jump witness orbits exactly). Original headline claim ("one-sided law") was corrected under referee pressure: the −5 family exceeds the coin model at j = 1, 2, 6, 7 (replicated on a disjoint range); the −1 spine is sub-coin at every measured j. **Null-model correction (same day, third referee):** the one-step Haar fresh law is exactly geometric, P(fresh ≥ j) = 2^(−j) — *proved* via the referee's branch-mass derivation (verified exhaustively mod 2²²: counts 2^(22−j) exactly, j = 1…13) — which falsified the memo's interim "a₅ ≥ 2 floor" explanation of the j = 1, 2 excess and localized all orbit deviations (excesses and deficits) to one open question: how positive orbits sample shallow residues vs Haar. Deep states (a ≥ 3) provably yield fresh = 0; the a = 2 boundary is mixed and sharp. One trivial deterministic bound proved (jump ≤ bitlen of the jump state; saturated exactly by both witnesses). Referee budget arithmetic shows a C ≥ 1 uniform tail bound alone cannot close divergence. Three referees total; the third also caught the memo misquoting its own ratio range. **Claims no part of the conjecture.**
